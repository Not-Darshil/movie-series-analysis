...
class CharacterNetworkGenerator:
    ...
    def generate_character_network(self,df):
        ...
        for row in df['ners']:
            previous_entries_in_window=[]
            
            for sentence in row:
                previous_entries_in_window.append(list(sentence))
                previous_entries_in_window = previous_entries_in_window [-windows:]
                
                # Flatten the 2D list into 1D List
                previous_entries_flattened = sum(previous_entries_in_window,[])
                
                for entity in sentence:
                    for entity_in_window in previous_entries_flattened:
    ...
    def draw_network_graph(self,relationship_df):
        ...
        allow-downloads" allowfullscreen="" allowpaymentrequest="" frameborder="0" srcdoc='{html}'></iframe>"""
